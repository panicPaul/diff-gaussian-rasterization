...
def _validate_inputs(scene: GaussianScene, camera: CameraState) -> None:
    ...
    if camera.cam_to_world.device.type != "cuda":
        raise ValueError("The Inria rasterizer requires camera tensors on CUDA.")
...
def _empty_like_scene(scene: GaussianScene, columns: int) -> Tensor:
    return torch.empty(
        (0, columns),
        dtype=scene.center_position.dtype,
        device=scene.center_position.device,
...
def _scene_feature_inputs(scene: GaussianScene) -> tuple[Tensor, Tensor]:
